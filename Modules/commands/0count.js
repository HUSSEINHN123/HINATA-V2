...
module.exports.run = async function ({ api, Threads, Users, event, args, client, __GLOBAL }) {
  ...
  if (input == "") { out(`أرجوك قم بإدخال بعض الفئات \n\nكيفية الإستعمال ؟\n${global.config.PREFIX}أحسب <الفئات>\n\nالفئات المتوفرة:\n\nالرسائل, المسؤولين, الأعضاء, ذكور, إماث, ألوان, الجميع, كل_المستخدمين, بيانات_المجموعة, عدد_المغادرات`) }
  ...
  if (input == "الأعضاء") { out(`هذه المجموعة لديها ${participantIDs.length} عضو`) }
  if (input == "ذكور") { out(`هذه المجموعة لديها ${gendernam.length} ذكر`) }
  if (input == "إناث") { out(`هذه المجموعة لديها ${gendernu.length} أنثى`) }
  if (input == "ألوان") { out(`هذه المجموعة لديا ${nope.length} عضو شاذ`) }
