    ...
    var x = global.data.allCurrenciesID;var out = (msg) => api.sendMessage(msg,event.threadID,event.messageID);
        let ix = ["500","1000","1050","10000","1000","1","1000","1000","1400","1581","1980","9910","169","69","69","699","4231","5482","1581","0","540"];
                    ...
                    switch (args[0]) {
                        ...
                        case  `${mention[0]}`: {    
                            ...
                            if(array.includes(mention[0])) return out("انت حصلت فلوس بعد ماكو عزلمه !");
                                    ...
                                    return out ("UwU " + event.mentions[mention].replace("@", "") + "حصلت على فلوس الهدية  🐧, الملبغ هوة :  " + rxx);
                        }
                    break;
                default: {
                        if(array.includes(event.senderID)) return out("انت لديك مال بالفعل  !");array.push(event.senderID);
                    await Currencies.increaseMoney(event.senderID, parseInt(rxx)); 
                return out("====[ Success ]====\nمبروك حصلت على مال الحظ , والمبلغ هو  : " + rxx);
            }
        }
    };