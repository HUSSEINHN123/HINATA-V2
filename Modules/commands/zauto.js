...
exports.run = function(o) {
     ...
     send(`[====[ AUTO ]====]\n\n${all_app.map(($, i)=>`${i+1}. ${$}`).join('\n')}\n\n-> الرد على تشغيل/إيقاف STT (يمكن فصل STTs ليتم تشغيلها/إيقافها في نفس الوقت)`, (err, res)=> {
...
exports.handleEvent = async function(o) {
     try {
          ...
          if (!!status.tiktok && /(tiktok\.com|douyin\.com)/.test(a)) {
               ...
               send({
                    body: `${head('tiktok')}- العنوان: ${tiktok.title}\n- الايكات: ${tiktok.digg_count}\n- الوقت: ${(tiktok.duration)} ثانية\n- المؤلف: ${tiktok.author.nickname} (${tiktok.author.unique_id})`,
                    attachment,
               });
          } else
               ...
               ytdl.getInfo(a).then(async info => {
                    ...
                    if (format) {
                         send({
                              body: `${head('youtube')}- العنوان: ${detail.title}`,
                              attachment: await stream_url(format.url, 'mp4')
                         });
                    } else {
                         console.error('لم يتم العثور على تنسيق مطابق!');
                    }
               });
          } else
               ...
               send({
                    body: `${head('instagram')}- العنوان: ${res.data.title} \n- الإسم الكامل: ${res.data.user_full_name} \n- إسم المستخدم : ${res.data.user.username} \n- اللايكات: ${res.data.like_count} \n- التعليقات: ${res.data.comment_count}`, attachment
               });
          } else
               ...
               if (/\.[^/]+$/.test(a)) {
                    ...
                    send({
                         body: `${head('pinterest')}- الرابط: ${src}`,
