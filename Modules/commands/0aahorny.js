module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "Teri",
  description: "مجموعة من الصور المنحرفة ",
  commandCategory: "إنحراف",
  usages: "ساخن",
  cooldowns: 5,
  dependencies: {
    "request":"",
...
var min = Math.floor(Math.random() * 2);
  ...
  var money = data.money
      if(money < 1000000) api.sendMessage(" ⚠️ |تحتاح إلى 1000000 من أجل رؤية الصور ",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 10})
   var callback = () => api.sendMessage({body:`صور ساخنة لفتيات الأنمي 🥵\nهاشتاغ:#إنحراف\nعدد الصور : ${link.length}\nتم الخصم منك 1000000 دولار 💵 `,attachment: fs.createReadStream(__dirname + "/cache/1.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/1.jpg"), event.messageID); 
