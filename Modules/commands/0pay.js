module.exports.config = {
    ...
    hasPermssion: 0,
    credits: "HINATA",
    description: "قم بتحويل المال - بالرد علة رسالة الشخص واكتب المبلغ",
    commandCategory: "تحويل أخوال",
...
const moment = require("moment-timezone");
    var time = moment.tz("Africa/Casablanca").format("HH:mm");
    var day = moment.tz("Africa/Casablanca").format("DD/MM/YYYY");
...
if(!isNaN(args[0])) {
    ...
    let bg = (await axios.get(`https://i.imgur.com/vFLTHzZ.jpg`, {responseType: "arraybuffer" })).data;
    ...
    ctx.fillStyle = "#31ddf7";
    ctx.textAlign = "start";
    ctx.fillText(args[0].replace(/\B(?=(\d{3})+(?!\d))/g, '.') + '$', 430, 165);
    ctx.font = "25px SplineSans";
    ctx.fillText(`${content}`, 547, 430);
    ctx.font = "26px SplineSans-Medium";
    ctx.textAlign = "right";
    ctx.fillText(`${userName}`, 550, 325);
    ctx.fillStyle = "#31ddf7";
    ...
    ctx.textAlign = "right";
    ctx.font = "22px SplineSans-Medium";
    ctx.fillText('15%', 117, 407);
    ctx.fillText('عملية ناجحة', 505, 405);
    ctx.fillText(`${time} - ${day}`, 640, 550);
    const imageBuffer = canvas.toBuffer();
    fs.writeFileSync(path, imageBuffer);
          return api.sendMessage(`[ دفع ] - يرجى الانتضار حتى اقوم بمعالجة المعاملة  💸`, event.threadID, async (err, info) => {
