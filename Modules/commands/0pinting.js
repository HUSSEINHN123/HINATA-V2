...
module.exports.config = {
  ...
  version: '3.0',
  hasPermission: 0,
  credits: '"Aleister Crowley"، الوصف: "تحويل النص إلى فن مذهل تم إنشاؤه بواسطة الذكاء الاصطناعي باستخدام نماذج نشر مستقرة.".',
  usePrefix: true,
  commandCategory: 'الذكاء الإصطناعي',
  usages: 'تخيلي2 [وصف]:[رقم النموذج]',
  cooldowns: 0,
};
...
module.exports.run = async function ({ api, event, args }) {
  try {
    ...
    if (prompt.includes(':')) {
      ...
      if (!isNaN(parsedModel) && parsedModel >= 0 && parsedModel < models.length) {
        ...
        return api.sendMessage(
          '❗ رقم نموذج غير صالح. يرجى تحديد رقم الموديل بين 0 و 43.',
          threadID,
          messageID
        );
      }
      ...
      return api.sendMessage(
        'يرجى تقديم وصف مع رقم الطراز إذا رغبت في ذلك.\n\nتخيلي2 {وصف}\nمثال: تخيلي فتاة جميلة\n\nتخيلي2 {وصف}:{رقم الموديل}\nمثال: تخيلي2 فتاة جميلة :43\n\n' +
        modelsList,
        threadID
      );
    }
    ...
    const processingMessage = await api.sendMessage(
      ' ⏱️ | جاري معالجة طلبك. انتظر من فضلك...',
      threadID,
      null,
      messageID
    );
    ...
    try {
      ...
      if (imageStream) {
        ...
        api.sendMessage('❌ حدث خطأ أثناء معالجة المطالبة الخاصة بك. الرجاء معاودة المحاولة في وقت لاحق', threadID, messageID);
      }
    } catch (error) {
