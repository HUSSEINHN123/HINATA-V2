module.exports.config = {
	...
	description: "قم بتشغيل مقاطع الفيديو عبر روابط يوتيوب أو كلمات البحث الرئيسية",
...
module.exports.handleReply = async function({ api, event, handleReply }) {
	...
	try {
		...
		ytdl(handleReply.link[event.body - 1])
			...
			.on("close", () => {
				if (statSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.m4a`).size > 26214400) return api.sendMessage('❗لا يمكن إرسال الملف لأن حجمه أكبر من 25 ميجابايت.', event.threadID, () => unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.m4a`), event.messageID);
	...
	if (urlValid) {
		try {
			...
			ytdl(args[0])
				...
				.on("close", () => {
					if (statSync(__dirname + `/cache/${id}.m4a`).size > 26214400) return api.sendMessage('❗لا يمكن إرسال الملف لأن حجمه أكبر من 25 ميجابايت.', event.threadID, () => unlinkSync(__dirname + `/cache/${id}.m4a`), event.messageID);
...
let getthumnail = (await axios.get(`${linkthumnail}`, { responseType: 'arraybuffer' })).data;
  ...
  imgthumnail.push(fs.createReadStream(__dirname + `/cache/${numb}.png`));
      ...
      var body = `»🔎 𝐂𝐨́ 【${link.length}】نتيجة تطابق كلمة بحثك \n\n🍑──── •🎶• ────🍑\n${msg}» قم بالرد على هذه الرسالة أعلاه بالرقم كن أجل تنزيل الأغنية `
