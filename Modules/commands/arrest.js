module.exports.config = {
    ...
    hasPermssion: 0,
    credits: "Joshua Sy",
    description: "قم بإراحة صديق قمت يعمل منشن له",
...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage(" ⚠️ | المرجو عمل منشن لشخص 1", threadID, messageID);
    else {
        var one = senderID, two = mention;
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "مبروك دخولك إلى مرتبات الدولة " + tag + '\n أتمنى لك السعادة',
            mentions: [{
          tag: tag,
          id: mention
        }],
