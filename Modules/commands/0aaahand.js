...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage(" ⚠️ |أرجوك قم بعمل منشن@", threadID, messageID);
    else {
        var one = senderID, two = mention;
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "أمسك يدي جيدا" + tag + ' لا تتركها 😍',
            mentions: [{
          tag: tag,
          id: mention
        }],
