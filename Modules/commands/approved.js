module.exports.config = {
  ...
  permission: 0,
  credits: "ryuko",
  description: "قم بالموافقة على أي مجموعة بإستخدام الآيدي",
  prefix: false,
  category: "المالك",
  usages: "موافقة [إضافة/إزالة] [آيدي المجموعة]",
  cooldowns: 5,
};
...
module.exports.languages = {
    ...
    "en": {
        "listAdmin": 'قائمة الموافقة : \n\n%1',
        "notHavePermssion": 'ليس ليك الإذن لإستخدام "%1"',
        "addedNewAdmin": 'تمت الموافقة على %1 من المجموعات :\n\n%2',
        "removedAdmin": 'تمت إزالة %1 من المجموعات في قائمة الموافقة :\n\n%2'
    }
}
        ...
        case "-a": {
            ...
            for (const idAdmin of listAdmin) {
                if (parseInt(idAdmin)) {
                  let boxname;
                  try {
        const groupName = await global.data.threadInfo.get(idAdmin).threadName || "الإسم غير موجود"
        boxname = `group name : ${groupName}\ngroup id : ${idAdmin}`;
      } catch (error) {
        const userName = await Users.getNameUser(idAdmin);
        boxname = `إسم المستخدم : ${userName}\nآيدي المستخدم : ${idAdmin}`;
      }
                  msg.push(`\n${boxname}`);
                }
            };

            return api.sendMessage(`تكت الموافقة على المجموعة و المستخدمين :\n${msg.join('\n')}`, threadID, messageID);
        }
        ...
        case "مجموعة": {
            ...
            if (mention.length != 0 && isNaN(content[0])) {
                ...
                config.APPROVED.push(content[0]);
                  ...
                  let boxname;
                  try {
        const groupname = await global.data.threadInfo.get(content[0]).threadName || "الإسم غير موجود";
        boxname = `إسم المجموعة : ${groupname}\nآيدي المجموعة : ${content[0]}`;
      } catch (error) {
        const username = await Users.getNameUser(content[0]);
        boxname = `إسم المستخدم : ${username}\nآيدي المستخدم : ${content[0]}`;
      }
                writeFileSync(configPath, JSON.stringify(config, null, 2), 'utf8');
                return api.sendMessage('هذهوالمجموعة تمت الموافقة عليها', content[0], () => {
        ...
        case "delete": {
            ...
            if (mentions.length != 0 && isNaN(content[0])) {
                ...
                config.APPROVED.splice(index, 1);
                  ...
                  let boxname;
                  try {
        const groupname = await global.data.threadInfo.get(content[0]).threadName || "هذه الإسم غير موجود";
        boxname = `إسم المجموعة : ${groupname}\nآيدي المجموعة : ${content[0]}`;
      } catch (error) {
        const username = await Users.getNameUser(content[0]);
        boxname = `إسم المستخدم : ${username}\nآيدي المستخدم : ${content[0]}`
                ...
                writeFileSync(configPath, JSON.stringify(config, null, 2), 'utf8');
                return api.sendMessage('هذه المجموعة تم حذفها من قائمة الموافقة', content[0], () => {
