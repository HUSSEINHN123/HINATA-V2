module.exports.config = {
	...
	description: "قم بإنشاء منشور للبوت",
  usePrefix: true,
...
module.exports.run = async ({ event, api }) => {
  ...
  return api.sendMessage(`اختر جمهورًا يمكنه رؤية مقالتك هذه\n1. الكل\n2. الأصدقاء\n3. فقط أنا`, threadID, (e, info) => {
...
const fs = require("fs-extra");
  ...
  if (type == "من_يرى") {
    if (!["1", "2", "3"].includes(body)) return api.sendMessage('يرجى اختيار 1 من العناصر الثلاثة أعلاه', threadID, messageID);
    formData.input.audience.privacy.base_state = body == 1 ? "الكل" : body == 2 ? "الأصدقاء" : "فقط_أنا";
    api.unsendMessage(handleReply.messageID, () => {
      api.sendMessage(`قم بالرد على هذه الرسالة بمضمون المقال، وإذا أردت تركها فارغة الرجاء الرد 0`, threadID, (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
    ...
    api.unsendMessage(handleReply.messageID, () => {
      api.sendMessage(`الرد على هذه الرسالة بصورة (يمكنك إرسال عدة صور، إذا كنت لا تريد نشر الصور، يرجى الرد 0) `, threadID, (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
