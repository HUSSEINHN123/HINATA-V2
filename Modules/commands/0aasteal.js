...
var msg = "";
    switch(handleReply.type) {
        case "choosee": {
            ...
            if (isNaN(event.body)) return api.sendMessage("⚡️أرجوك قم بإدخال رقم واحد", event.threadID, event.messageID);
            if (choose > 7 || choose < 1) return api.sendMessage("⚡️هءه الخيار ليس في القائمة.", event.threadID, event.messageID); //thay số case vào số 7
...
module.exports.run = async ({  event, api, handleReply, Currencies, getText }) => {
  ...
  /*cave*/    "\n6. 1000000$" +
                "" +
                "\n\n⚡️بينزو كم بدك تسحب ؟ إختر رقم" //add case display here ||  \n[number]. [Career]" +
