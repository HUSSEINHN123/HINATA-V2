module.exports.config = {
    ...
    commandCategory: "حب",
    usages: `المرجو عمل منشن لشحص واحد\n\nكيغية الإستخدام ؟\n${global.config.PREFIX}قبلة <@تاغ>\n\nمثال:\n${global.config.PREFIX}قبلة @الإسم\n`,
    cooldowns: 5,
    dependencies: {
        "axios": "",
        "fs-extra": "",
...
module.exports.run = async function ({ event, api, args, Currencies }) { 
  ...
  if (!two) return api.sendMessage(`المرجو عمل منشن لشحص واحد\n\nكيفية الإستخدام ؟\n${global.config.PREFIX}قبلة <@تاغ>\n\nمثال:\n${global.config.PREFIX}قبلة @الإسم\n\nتم التعديل من طرف: حسين يعقوبي`, threadID, messageID);
  else {
        return makeImage({ one, two }).then(path => api.sendMessage({ body: `${ae[Math.floor(Math.random() * ae.length)]}\nتعاطفك بعد أن تمت سرقتك هو ${hc} %\n + ${((hc)*rd)} دولار`, attachment: fs.createReadStream(path)}, threadID, () => fs.unlinkSync(path), messageID));
  }
                                                                                                   }