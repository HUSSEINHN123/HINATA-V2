module.exports.config = {
  ...
  description: "حظر الأعضاء نهائيًا من المجموعة (تذكر تعيين بوت )\nالنظام و رفع الحظر عنهم أيضا",
  commandCategory: "المجموعة",
  usages: "[مفتاح]",
  cooldowns: 5,
  info: [
    {
      key: '[تاغ@] أو [رد على رسالة] "السبب"',
...
module.exports.run = async function({ api, args, Users, event, Threads, utils, client }) {
  ...
  if (!info.adminIDs.some(item => item.id == api.getCurrentUserID())) return api.sendMessage('يحتاج البوت إلى حقوق مسؤول المجموعة لاستخدام هذا الأمر\nيرجى  الإضافة والمحاولة مرة أخرى!', threadID, messageID);
  ...
  */
  ...
  if(args[0] == "عرض") {
    if(!args[1]) {
      ...
      if(!mywarn) return api.sendMessage('✅ | أنت لم يتم حظرك من قبل', threadID, messageID);
      var num = 1;
      for(let reasonwarn of mywarn) {
        msg += `reasonwarn\n`;
      }
      api.sendMessage(`❎ |لقد تم حظرك لسبب : ${msg}`, threadID, messageID);
    }
      ...
      allwarn == "" ? api.sendMessage("✅ |لا أحد فيكم تم تحذيره بعد", threadID, messageID) : api.sendMessage("قائمة الأعضاء اللذين تم حظرهم:\n"+allwarn, threadID, messageID);
    }
  }

  else if(args[0] == "رفع_الحظر") {
    var id = parseInt(args[1]), mybox = bans.banned[threadID];
    var info = await api.getThreadInfo(threadID);
  if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('❎ | لم يتم تحديد اي شخص تم حظره!', threadID, messageID);

    if(!id) return api.sendMessage("❎ | تحتاج لإدخال الآيدي لهذا الشخص من أجل إزالته من قائمة المحظورين", threadID, messageID);
    bans.banned;
    if(!mybox.includes(id)) return api.sendMessage("✅ |لم يتم حظر هذا الشخص من مجموعتك بعد", threadID, messageID);
      api.sendMessage(`✅ | تمت إزالة آيدي الشخص ${id} من قائمة المحظورين بنجاح !`, threadID, messageID);
    ...
    var info = await api.getThreadInfo(threadID);
  if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('❎ | من الصعب تنفيذ الأمر ', threadID, messageID);
    ...
    api.sendMessage("Reset all data in your group", threadID, messageID);
     ...
     //◆━━━━━━━━━◆تحذير◆━━━━━━━━━◆\\
       ...
       //◆━━━━━━◆قم بالحصول على آيدي المستخدم مع السبب <<<<<<<<\\
       var info = await api.getThreadInfo(threadID);
  if (!info.adminIDs.some(item => item.id == senderID) && !(global.config.ADMINBOT).includes(senderID)) return api.sendMessage('من الصعب ❓', threadID, messageID);
  var reason = "";
    ...
    for(let iid of iduser) {
      ...
      if(!reason) reason += " ❓ | لم يتم إدخال أي سبب";
      /*if(!bans.warns.hasOwnProperty(threadID)) {
      bans.warns[threadID] = {}; 
      }*/
    ...
    api.sendMessage({body: ` ⛔ | تم حظر العضو ${arrayname.join(", ")} يجب عليه ترك المجموعة بشكل دائم و السبب : ${reason}`, mentions: arraytag}, threadID, messageID);
    fs.writeFileSync(__dirname + `/cache/bans.json`, JSON.stringify(bans, null, 2));
}
