...
module.exports.run = async ({ api, event,args }) => {  {
    ...
    const request = global.nodemodule["request"];
	 ...
	 let text = args.join(" ")
  if (!text) return api.sendMessage('أرجوك قم بإدخاله على هذه الصيغة [نص1 | نص2 ]!', event.threadID, event.messageID);
  const length_0 = parseInt(text.length)
  const text1 = text.substr(0, text.indexOf(' | ')); 
  if (!text1) return api.sendMessage('أرحوك قم بإدخالها على هذه الصيغة [نص1 | نص2 ]!', event.threadID, event.messageID);
  const length = parseInt(text1.length)
  const text2 = text.split(" | ").pop()
  if (!text2) return api.sendMessage('أرجوك قم بإدخالها على هذه الصيغة [نص1 | نص2 ]!', event.threadID, event.messageID);
