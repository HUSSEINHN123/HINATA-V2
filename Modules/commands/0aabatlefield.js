...
module.exports.run = async function({api, event, args, utils, Users, Threads}) {
    try {
       ...
       let text = args.join(" ")
  if (!text) return api.sendMessage(' ⚠️ |أرجوك قم بإدخال النص بشكل صحيح [نص1 | نص2]!', event.threadID, event.messageID);
  const length_0 = parseInt(text.length)
  const text1 = text.substr(0, text.indexOf(' | ')); 
  if (!text1) return api.sendMessage(' ⚠️ |أرجوك قم بإدخال النص بشكل صحيح [نص1 | نص2]!', event.threadID, event.messageID);
  const length = parseInt(text1.length)
  const text2 = text.split(" | ").pop()
  if (!text2) return api.sendMessage(' ⚠️ |أرجوك قم بإدخال النص بشكل صحيح [نص1 | نص2]!', event.threadID, event.messageID);
  const length_2 = parseInt(text2.length)
	...
	let callback = function() {
        ...
        console.log(err)
        return api.sendMessage(` ❌ | خطأ`, event.threadID)
    }
}