...
module.exports.config = {
	...
	hasPermssion: 0,
	credits: "ProCoderMew",
	description: "قم بإضافة عن طريق رايط المجموعة أو الآيدي",
...
module.exports.run = async function ({ api, event, args }) {
	...
	if (!args[0]) return out("المرجو إدخال آيدي واحد/رابط الملف الشخصي للمستخدم.");
	if (!isNaN(args[0])) return adduser(args[0], undefined);
	else {
		try {
			var [id, name, fail] = await getUID(args[0], api);
			if (fail == true && id != null) return out(id);
			else if (fail == true && id == null) return out("لم يتم العثور على آيدي المستخدم.")
			else {
	...
	async function adduser(id, name) {
		id = parseInt(id);
		if (participantIDs.includes(id)) return out(`${name ? name : "العضو"} هو موجود بالفعل في المجموعة.`);
		else {
			...
			try {
				...
				return out(`لا يمكنني إضافة ${name ? name : "user"} إلى المجموعة.`);
			}
			if (approvalMode === true && !admins.includes(botID)) return out(`تم إضافة ${name ? name : "العضو"} إلى قائمة الموافقة!`);
			else return out(`تمت إضافة ${name ? name : "العضو"} إلى المجموعة بنجاح ✅!`)
		}
	}
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 }