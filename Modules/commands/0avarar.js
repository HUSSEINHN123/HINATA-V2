...
module.exports.run = async function({ api, args, event, permssion }) {
  ...
  if(args[0] == "find" || args[0] == "tìm"){
    ...
    var msg = ({
      body: `آيدي الشخصية ${args[1]}, DEFAULT COLOR ${lengthchar[args[1]].colorBg}`,
      attachment: t
    })
    ...
    var count = alime.listAnime.length;
      ...
      msg += `صفحة (${page}/${numPage})\nإستخدم ${global.config.PREFIX}${this.config.name} قائمة <رقم الصفحة>`;
      return api.sendMessage(msg, event.threadID,event.messageID);
  } else {
  ...
  if (!fs.existsSync(__dirname +
   ...
   return api.sendMessage("قم بالرد على هذه الرسالة من أجل إختيار آيدي الشخصية", event.threadID, (err, info) => {
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  if (event.senderID != handleReply.author) return api.sendMessage('بالتوفيق!', event.threaID);
  ...
  const msg = ({
    body: `لقد اخترت الحرف الذي يحمل الرقم التسلسلي ${event.body}, قم بالرد على هذه الرسالة لاختيار إطار لون الصورة`,
     attachment: o
  })
  ...
  const msg = ({
    body: `لقد حددت رقم الإطار${event.body}, هذا من أجل للاسم الرئيسي`,
     attachment: z
  })
    return api.sendMessage(msg, event.threadID, (err, info) => {
      if(err) return api.sendMessage(`خطأ في الإنشاء`, event.threadID)
      return global.client.handleReply.push({
        step: 3,
        name: this.config.name,
        author: event.senderID,
     ...
     api.unsendMessage(handleReply.messageID);
    return api.sendMessage(`لقد قمت بإختيار الإسم الرئيسي اللذي يحمل إسم ${event.body}, قم بالرد على هذه الرسالة من أجل الإستمرا و إدخال الاسم الفرعي`, event.threadID, (err, info) => {
      if(err) return api.sendMessage(`خطأ في الإنشاء`, event.threadID)
      return global.client.handleReply.push({
        step: 4,
        name: this.config.name,
     ...
     api.unsendMessage(handleReply.messageID);
    return api.sendMessage(`لقد قمت بإختيار الإسم تلثانوي هو ${event.body}, قم بالرد على الرسالة للإستمرار في هذه الرسالة لإدخال اسم المستخدم على إنستجرام`, event.threadID, (err, info) => {
      if(err) return api.sendMessage(`خطأ في الإنشاء`, event.threadID)
      return global.client.handleReply.push({
        step: 5,
        name: this.config.name,
     ...
     api.unsendMessage(handleReply.messageID);
    return api.sendMessage(`لقد قكت بإدخال إسم المستخدم على إنستغرام ك ${event.body}, قم بالرد على هذه الرسالة و فم بإدحال إسم المستخدم على جيت هيوب (GitHub)`, event.threadID, (err, info) => {
      if(err) return api.sendMessage(`خطأ في الإنشاء`, event.threadID)
      return global.client.handleReply.push({
        step: 6,
        name: this.config.name,
     ...
     api.unsendMessage(handleReply.messageID);
    return api.sendMessage(`ل قمت لتو بإدحال إسم المستخدم على GitHub واللذي هو${event.body}, قم بالرد على هذه الرسالة بإسم المستخدم على فيسبوك`, event.threadID, (err, info) => {
      if(err) return api.sendMessage(`خطأ في الإنشاء`, event.threadID)
      return global.client.handleReply.push({
        step: 7,
        name: this.config.name,
    ...
    api.unsendMessage(handleReply.messageID);
    return api.sendMessage(`لقد قمت بإدخال إسم مستخدم المستخدم على فيسبوك ك ${event.body}, قم بالرد على هذه الرسالة من أجل إدخال اللون الذي تريده يرجى كتابة لا إذا كنت تريد استخدام اللون الافتراضي`, event.threadID, (err, info) => {
      if(err) return api.sendMessage(`Uncertain error`, event.threadID)
      return global.client.handleReply.push({
        step: 8,
        name: this.config.name,
