...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("المرجو عمل منشن 🙃.", threadID, messageID);
    else {
        var one = senderID, two = mention;
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "أمسكي بيدي يا حبيبتي" + tag + ' ولا تتركيها أحبك😍',
            mentions: [{
          tag: tag,
          id: mention
        }],
